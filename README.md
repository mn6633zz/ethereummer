...
This is full source of Ethereum Mining Pool - http://ethereumpool.co Pool has been written mostly in php. Pool software is not bad, but needs some work, it runs fine on on decent dedicated server if it's not more than ~500 workers, for more it needs to be rewritten and split between more servers. It should be considered alpha code. Code is messy, my intention at time was to swiftly create pool to save community from only one existing pool with more than 3/4 network hashrate.
